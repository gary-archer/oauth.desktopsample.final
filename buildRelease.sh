...
NODE_OPTIONS='--import tsx' BUILD=release npx rollup --config build/rollup.main.config.ts
if [ $? -ne 0 ]; then
...
NODE_OPTIONS='--import tsx' BUILD=release npx rollup --config build/rollup.renderer.config.ts
if [ $? -ne 0 ]; then
